...
| `tests/smoke-test.ps1` | 12 проверок бриджа на фейковых `git`/`gh` (без сети) |
...
Ожидаете `ALL SMOKE TESTS PASSED` (12 проверок). Тест создаёт временную
песочницу и фейковые `git`/`gh`, реальный репозиторий и GitHub не трогает.
