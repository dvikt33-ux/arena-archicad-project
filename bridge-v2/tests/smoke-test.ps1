...
if ($isWin) {
    ...
    Write-AsciiCrlf (Join-Path $shimDir 'git.cmd') @(
        ...
        'exit /b 1',
        ':status',
        'if not "%FAKE_GIT_STATUS_FILE%"=="" if exist "%FAKE_GIT_STATUS_FILE%" goto statusfile',
        'echo On branch fake',
        'echo nothing to commit, working tree clean',
        'exit /b 0',
        ':statusfile',
        'type "%FAKE_GIT_STATUS_FILE%"',
        'exit /b 0',
        ':gitlog',
    ...
    $gitSh = @(
        ...
        '  status)',
        '    if [ -n "$FAKE_GIT_STATUS_FILE" ] && [ -f "$FAKE_GIT_STATUS_FILE" ]; then cat "$FAKE_GIT_STATUS_FILE"; exit 0; fi',
        '    echo "On branch fake"; echo "nothing to commit, working tree clean"; exit 0;;',
