...
| `mailbox.ps1` | Приём удалённых заданий. По умолчанию выключен |
| `arena-mailbox-put.ps1` | Локальная постановка action-ID в очередь, без текста команды |
| `MAILBOX.md` | Граница доверия обратного канала |
| `AUTONOMY.md` | Будущий цикл правок. Helper не реализован |
...
- **Durable outbox**: GitHub недоступен → результат лежит в очереди и
  ...
  Id комментария GitHub больше не влезает в Int32; он хранится строкой.
  Успешный `gh` (exit 0) не повторяется, если id распознан или маркер уже есть
  в Issue — иначе POST создавал дубликаты.
...
- Обратный канал выключен, пока нет локального `%LOCALAPPDATA%\ArenaBridge\mailbox.json`.
  Публичный Issue не источник команд. Секрет модели не передаётся.
  Разрешение — права приватного mailbox, не автор коммита. Подробности: `MAILBOX.md`.
  Список collaborators и deploy keys читается постранично и при обрыве отказывает.
  Это не доказательство единственного writer и не доказательство происхождения файла.
- Публикация по-прежнему в публичный Issue #1. `git diff` не публикуется.
  `git status` публикуется только как сводка без путей и сырого вывода.
- Задачи из GitHub не подписываются секретом модели: секрета в чате нет.
  Локальный inbox по-прежнему может писать локальный процесс (M-8).
  Будущий цикл — `AUTONOMY.md`. Повышенный helper не реализован.
